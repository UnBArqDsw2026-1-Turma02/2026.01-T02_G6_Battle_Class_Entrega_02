...
O Diagrama de Classes foi elaborado por **Lucas Oliveira** em draw.io. Pela quantidade de classes e relacionamentos, a imagem completa ficou pequena em tela única — por isso, o arquivo foi recortado em quatro quadrantes sequenciais para leitura detalhada. A versão inteira continua disponível em [`DiagramaDeClasse.png`](../assets/diagramas/estatica/DiagramaDeClasse.png).

![Diagrama de Classes (visão geral)](../assets/diagramas/estatica/DiagramaDeClasse.png)

**Quadrante superior esquerdo — Tower Defense.** Hierarquia `Torre` (abstrata) ← `TorreComum` / `TorreEspecial`, com composição de `UpgradeArvore` (árvore de upgrades recursiva) e a raiz `Area` da taxonomia.

![Classes — Quadrante Superior Esquerdo](../assets/diagramas/estatica/DiagramaDeClasseTopLeft.png)

**Quadrante superior direito — Economia, Ranking e Partida.** `Carteira` com `MoedaQuestao` (datatype persistente) e `MoedaOuroPartida` (datatype volátil); `Estudante` associado à `Carteira`, ao `Ranking` e à `Partida`; `Mapa` associado à partida.

![Classes — Quadrante Superior Direito](../assets/diagramas/estatica/DiagramaDeClasseTopRight.png)

**Quadrante inferior esquerdo — Conhecimento.** Hierarquia de `Taxonomia` (abstrata) com `Linha`, `Disciplina` e `Tema`; `Prova` agrega `Questao` que tem 5 `Alternativa` e `Feedback` associado; `Roleta` como mecanismo de sorteio.

![Classes — Quadrante Inferior Esquerdo](../assets/diagramas/estatica/DiagramaDeClasseBottomLeft.png)

**Quadrante inferior direito — Identidade e Gerações.** Hierarquia de `Usuario` (abstrata) com `Estudante`, `Professor` e `Admin`; `Heroi` e `Onda` como elementos da partida; `IAGeradoraQuestoes` implementando a interface `IGeradorConteudo`.

![Classes — Quadrante Inferior Direito](../assets/diagramas/estatica/DiagramaDeClasseBottomRIght.png)
...
| -------------- | ---------------------------------------------------------------------------------------- |
| [Ana Elisa](/Modelagem/2.4.ParticipacoesModelagem?id=ana-elisa) | ![Ana — Classes](../assets/comprovatorios/Ana/ComentatioDiagramaDeClasses.png)           |
| [Gabriela Tiago](/Modelagem/2.4.ParticipacoesModelagem?id=gabriela) | ![Gabriela — Classes](../assets/comprovatorios/Gabriela/ComentarioDiagramaDeClasses.png) |
| [Lucas Oliveira](/Modelagem/2.4.ParticipacoesModelagem?id=lucas) | ![Lucas — Classes (autor + resposta à Ana)](../assets/comprovatorios/Lucas/ComentarioDiagramaDeClasse.png) |
...
| Membro         | Comentário / evidência                                                                                      |
| -------------- | ----------------------------------------------------------------------------------------------------------- |
| [Ana Elisa](/Modelagem/2.4.ParticipacoesModelagem?id=ana-elisa) | ![Ana — Componentes](../assets/comprovatorios/Ana/ComentarioDiagramaDeComponentes.png)                      |
| [João Sapiência](/Modelagem/2.4.ParticipacoesModelagem?id=joao-sapiencia) | ![João Sapiência — Componentes](../assets/comprovatorios/JoaoSapiencia/ComentarioDiagramaDeComponentes.png) |
| [Marina](/Modelagem/2.4.ParticipacoesModelagem?id=marina) | ![Marina — Componentes](../assets/comprovatorios/Marina/ComentarioDiagramaDeComponentes.png)                |
| [Otávio](/Modelagem/2.4.ParticipacoesModelagem?id=otavio) | ![Otávio — Componentes](../assets/comprovatorios/Otavio/ComprobatorioDiagramaDeComponentes.png)             |
| [Thiago](/Modelagem/2.4.ParticipacoesModelagem?id=thiago) | ![Thiago — Componentes](../assets/comprovatorios/Thiago/ComentarioComponentes.PNG)                          |
...
- **Por que Kubernetes e não serverless, mesmo no MVP?** A escolha é movida por quatro fatores concretos, nessa ordem:
  - **Escalabilidade desde o dia um.** O projeto é pensado para crescer rápido — concentrar questões, usuários e partidas simultâneas implica escalar horizontalmente sob demanda, em especial o `MotorTowerDefense`, que é sensível a latência. Kubernetes expõe esse controle (`HorizontalPodAutoscaler`, *ingress* com *load balancing*) já no MVP.
  - **Custo sob controle no longo prazo.** Serverless tende a ficar mais caro conforme o volume de requisições sobe. Com *pods* dimensionados por HPA em um cluster dedicado, conseguimos prever custo mensal e aproveitar *commit discounts* dos provedores — algo que seria inviável com preço por invocação.
  - **Expertise disponível (ainda que não difundida).** Alguns membros da equipe têm experiência prévia em Kubernetes (manifestos, Ingress, Helm) — não é expertise difundida por todos, mas é suficiente para destravar a adoção sem precisar formar o time inteiro antes. Esses membros atuam como referência técnica durante o MVP, enquanto o restante da equipe aprende conforme necessário.
  - **Infraestrutura disponível.** Já temos máquinas prontas para o cluster, então o *setup* não introduz custo adicional de provisionamento.

  Serverless também traria *cold start* incompatível com a latência esperada do `MotorTowerDefense`, o que reforça a decisão.
...
| Membro         | Comentário / evidência                                                                                                |
| -------------- | --------------------------------------------------------------------------------------------------------------------- |
| [Ana Elisa](/Modelagem/2.4.ParticipacoesModelagem?id=ana-elisa) | ![Ana — Implantação](../assets/comprovatorios/Ana/ComentarioDiagramaDeImplantacao.png)                                |
| [Dannyeclisson](/Modelagem/2.4.ParticipacoesModelagem?id=dannyeclisson) | ![Dann — Implantação](../assets/comprovatorios/Dannyeclisson/ComentarioDiagramaDeImplantacao.png)                     |
| [João Sapiência](/Modelagem/2.4.ParticipacoesModelagem?id=joao-sapiencia) | ![João Sapiência — Implantação](../assets/comprovatorios/JoaoSapiencia/ComentarioDiagramaDeImplantacao.png)           |
| [Lucas Oliveira](/Modelagem/2.4.ParticipacoesModelagem?id=lucas) | ![Lucas — Implantação](../assets/comprovatorios/Lucas/ComentarioDiagramaDeImplantacao.png)                            |
| [Otávio](/Modelagem/2.4.ParticipacoesModelagem?id=otavio) | ![Otávio — Implantação (histórico de edição)](../assets/comprovatorios/Otavio/ComprobatorioDiagramaDeImplantacao.png) |
| [Thiago](/Modelagem/2.4.ParticipacoesModelagem?id=thiago) | ![Thiago — Implantação](../assets/comprovatorios/Thiago/ComentarioImplantacao.PNG)                                    |
...
| ------ | ---- | --------- | --------- | ----------- |
| 1.0 | 24/04/2026 | Consolidação dos 3 diagramas estáticos e comprobatórios | João Sapiência, Marina, Ana Elisa, Thiago, João Lobo, Otávio | Equipe G6 |
| 1.1 | 24/04/2026 | Diagrama de Classes publicado por Lucas Oliveira (visão geral + 4 quadrantes ampliados) | [Lucas Oliveira](/Modelagem/2.4.ParticipacoesModelagem?id=lucas) | Ana Elisa, Gabriela |
